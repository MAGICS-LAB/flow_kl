"""
Evaluation routines for learned density q_t (Part-2 Learning version).
...
- compute_epsilon_learn: Compute ε_θ (RMS flow-matching loss for learned model)
- compute_score_gap_integral_learn: Compute S_θ (score-gap integral for learned model)
- compute_kl_at_t1_learn: Compute KL(p_1|q^θ_1) for learned model
"""
...
def divergence_of(velocity, x, t, need_grad_wrt_x=False):
    ...
    Args:
        ...
        t: Time point(s)
        need_grad_wrt_x: If True, build graph for later differentiation w.r.t. x (for score computation)
    ...
    # Divergence always requires gradients (it's a first-order derivative)
    # but we only build higher-order graph if needed for score computation
    
    # Ensure we can compute gradients even if x is detached
    x_req = x.detach().clone().requires_grad_(True)
    
    # Use enable_grad context to allow gradient computation even if we're in no_grad
    with torch.enable_grad():
        v = velocity.forward(x_req, t)  # (..., dim)
        
        # Sum_i ∂v_i/∂x_i
        create_graph = need_grad_wrt_x
        grads = []
        for i in range(v.shape[-1]):
            gi = torch.autograd.grad(
                v[..., i].sum(), x_req, create_graph=create_graph, retain_graph=True
            )[0][..., i:i+1]
            grads.append(gi)
...
def backward_ode_and_divergence(v_theta, x, t, rtol=1e-6, atol=1e-8, need_grad_for_score=False):
    """
    Solve backward ODE dx/ds = -v_θ(x(s),s) from s=t down to s=0.
    Simultaneously accumulate divergence ℓ = ∫₀ᵗ ∇·v_θ(x(s),s) ds.
    
    Args:
        v_theta: Velocity object (MLP or synthetic)
        x: Terminal points of shape [B, 2]
        t: Terminal time (scalar or tensor of shape [B])
        rtol: Relative tolerance for ODE solver
        atol: Absolute tolerance for ODE solver
        need_grad_for_score: If True, build graph for later score computation
    ...
    # Detach x if we don't need gradients
    if not need_grad_for_score:
        x = x.detach()
    ...
    def ode_func(s, z):
        ...
        divergence = divergence_of(v_theta, x_s, s, need_grad_wrt_x=need_grad_for_score).squeeze(-1)
...
def log_q_t(x, t, v_theta, schedule, rtol=1e-6, atol=1e-8, need_grad_for_score=False):
    """
    ...
    # Backward ODE
    x_0, ell = backward_ode_and_divergence(v_theta, x_batch, t, rtol, atol, need_grad_for_score=need_grad_for_score)
...
def score_q_t(x, t, v_theta, schedule, rtol=1e-6, atol=1e-8):
    ...
    # Enable gradients (required for score computation)
    x_grad = x_batch.clone().detach().requires_grad_(True)
    
    # Compute log q_t(x) as scalar (with gradient graph built)
    log_q = log_q_t(x_grad, t, v_theta, schedule, rtol, atol, need_grad_for_score=True)
    
    # Compute gradient via autograd
    score = torch.autograd.grad(log_q.sum(), x_grad, create_graph=False)[0]
...
def compute_epsilon_learn(v_theta, schedule, val_times=101, val_samples_per_time=4096, 
                          device='cpu', dtype=torch.float64):
    """
    Compute ε_θ = √(E|v_θ - u|²) for learned velocity model.
    
    Uses validate_model to compute validation MSE, then takes sqrt.
    
    Args:
        v_theta: Trained velocity model (MLP)
        schedule: Schedule enum
    ...
    Returns:
        epsilon_theta (scalar)
    """
    val_mse, val_nmse = validate_model(
        v_theta, schedule, val_times, val_samples_per_time, device, dtype
    )
    return math.sqrt(val_mse)


def compute_score_gap_integral_learn(v_theta, schedule, K_S=101, N_S=2048,
                                      rtol=1e-6, atol=1e-8, eval_seed=12345,
                                      device='cpu', dtype=torch.float64, chunk_size=1024):
    """
    Compute S_θ = ∫₀¹ E|s_p - s_q^θ|² dt for learned velocity model.
    
    Uses common random numbers and chunked processing for memory efficiency.
    ...
    Args:
        ...
        N_S: Samples per time point
        rtol, atol: ODE solver tolerances
        eval_seed: Random seed for CRNs
        device: torch device
        dtype: torch dtype
        chunk_size: Batch size for processing (default 1024)
    ...
    for t_val in t_grid:
        # Sample x using common random numbers
        x_all = sample_p_t_with_crn(t_val.item(), N_S, schedule, Z, dtype).to(device)
        
        # Compute analytic score
        sp = score_p_t(x_all, t_val.item(), schedule)  # (-x/σ_p^2)
        
        # Compute learned score in chunks
        sq_chunks = []
        num_chunks = (N_S + chunk_size - 1) // chunk_size
        
        for chunk_idx in range(num_chunks):
            start_idx = chunk_idx * chunk_size
            end_idx = min(start_idx + chunk_size, N_S)
            x_chunk = x_all[start_idx:end_idx].detach()  # Ensure detached
            
            # Compute score (with gradients enabled for score computation)
            x_chunk_grad = x_chunk.clone().requires_grad_(True)
            sq_chunk = score_q_t(x_chunk_grad, t_val.item(), v_theta, schedule, rtol, atol)
            # Detach and move to CPU if needed (for memory management)
            sq_chunk_detached = sq_chunk.detach()
            if isinstance(device, torch.device) and device.type == 'cuda':
                sq_chunk_detached = sq_chunk_detached.cpu()
            sq_chunks.append(sq_chunk_detached)
            
            # Clear graph between chunks
            del x_chunk_grad, sq_chunk
        
        # Concatenate and ensure on correct device
        sq = torch.cat(sq_chunks, dim=0)
        if sq.device != device:
            sq = sq.to(device)
        ...
        # Clear graph between times
        del x_all, sp, sq, sq_chunks
...
def compute_kl_at_t1_learn(v_theta, schedule, N_kl=20000, rtol=1e-6, atol=1e-8,
                           eval_seed=12345, device='cpu', dtype=torch.float64, chunk_size=1024):
    """
    Compute KL(p_1|q^θ_1) at t=1 for learned velocity model.
    ...
    # Sample x ~ p_1
    x_all = sample_p_t(1.0, N_kl, schedule, device=device, dtype=dtype)
    
    # Process in chunks (no gradients needed for KL computation)
    kl_chunks = []
    num_chunks = (N_kl + chunk_size - 1) // chunk_size
    
    with torch.no_grad():
        for chunk_idx in range(num_chunks):
            start_idx = chunk_idx * chunk_size
            end_idx = min(start_idx + chunk_size, N_kl)
            x_chunk = x_all[start_idx:end_idx]
            
            # Compute log densities (no gradients needed)
            lp_chunk = log_p_t(x_chunk, 1.0, schedule)
            lq_chunk = log_q_t(x_chunk, 1.0, v_theta, schedule, rtol, atol, need_grad_for_score=False)
            
            kl_chunk = lp_chunk - lq_chunk
            kl_chunks.append(kl_chunk)
    
    # Aggregate
    kl_all = torch.cat(kl_chunks, dim=0)
    return kl_all.mean().item()
