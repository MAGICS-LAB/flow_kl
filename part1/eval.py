"""
Evaluation routines for learned density q_t.
...
def backward_ode_and_divergence(v_theta, x, t, rtol=1e-6, atol=1e-8):
    ...
    def ode_func(s, z):
        ...
        # Compute divergence
        x_grad = x_s.clone().detach().requires_grad_(True)
        v_grad = v_theta(x_grad, s)

        divergence = torch.zeros_like(v_grad[..., 0])
        for i in range(2):
...
if __name__ == '__main__':
    ...
    from core.true_path import Schedule, velocity_u
    from part1.train import train_velocity_model
    ...
    from torchdiffeq import odeint as odeint_forward
    ...
    x_forward_final = odeint_forward(forward_ode_func, x_0_rev, torch.tensor([0.0, t], dtype=torch.float64))[-1]
