...
Usage:
    python -m part1.experiment --schedule a1 --seed 42
    python -m part1.experiment --schedule a2
    python -m part1.experiment --schedule a3 --num_samples 4000 --num_times 201
"""
...
from core.true_path import Schedule, schedule_to_enum, sample_p_t
from part1.train import train_velocity_model, validate_model
from part1.eval import compute_kl_lhs, compute_rhs_integrand, integrate_rhs
...
def main():
    ...
    kl_curve_mean = kl_curve_all.mean(axis=0)
    rhs_integrand_mean = rhs_integrand_all.mean(axis=0)
    rhs_cumulative_mean = rhs_cumulative_all.mean(axis=0)

    kl_curve_std = kl_curve_all.std(axis=0) if args.num_seeds > 1 else None
    rhs_integrand_std = rhs_integrand_all.std(axis=0) if args.num_seeds > 1 else None
    rhs_cumulative_std = rhs_cumulative_all.std(axis=0) if args.num_seeds > 1 else None

    metrics = compute_relative_error(rhs_cumulative_mean, kl_curve_mean)
    print(f"\nRelative error stats (RHS vs LHS): median={metrics['median']:.2f}%, max={metrics['max']:.2f}%, mean={metrics['mean']:.2f}%")
    ...
    metadata = {
        ...
        'target_mse': args.target_mse,
        'target_nmse': args.target_nmse,
        'model_path': model_path if args.load_model is None else args.load_model,
        'metrics': metrics,
    }
